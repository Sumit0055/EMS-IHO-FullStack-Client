...
//                 <span className='bg-white/90 backdrop-blur-sm px-2.5 py-1 text-xs 
...
const EmployeeCard = ({ employee, onDelete, onEdit }) => {
  ...
  return (
    <div className="group relative card card-hover overflow-hidden">
      <div className="relative aspect-4/3 w-full overflow-hidden bg-gradient-to-br from-sky-100 to-sky-50">
        <div className="w-full h-full flex items-center justify-center">
          {/* circle icon */}
      ...
      {!employee.isDeleted && (
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-indigo-700/20 via-transparent to-transparent 
  opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity flex items-end justify-center pb-6 gap-3 mb-3">
          <button
            onClick={() => onEdit(employee)}
            className="p-2.5 bg-white/90 backdrop-blur-sm text-slate-700 hover:text-indigo-600 rounded-xl shadow-lg transition-all hover:scale-105 ms-50 "
          >
            <Pencil className="w-4 h-4" />
          </button>
          <button
