    ...
    return (
        <form action="" onSubmit={handleSubmit} className='card p-5 sm:p-6 mb-6'>
            <h2 className='text-base font-medium text-slate-900 mb-6 pb-4 border-b border-slate-100 flex items-center gap-2'>
              <User className='w-5 h-5 text-slate-400'/>  Public Peofile
            </h2>
            {error && (
                <div className="bg-red-50 text-rose-700 p-4 rounded-xl text-sm border border-rose-200 mb-6 flex items-center gap-2">
                    <div className='w-1.5 h-1.5 rounded-full bg-red-500 mt-1.5 shrink-0'/>
                    {error}
                </div>
            )}
            {message && (
                <div className="bg-emerald-50 text-emerald-700 p-4 rounded-xl text-sm border border-emerald-200 mb-6 flex-start gap-3">
            ...
            <div className="space-y-5">
                ...
                {initialData.isDeleted ? (
                    <div className="pt-2">
                        <div className="p-4 bg-rose-50 border-b-rose-200 rounded-xl text-center">
                            <p className='text-rose-600 font-medium tracking-tight'>Account Deactivated</p>
                            <p className='text-sm text-rose-500 mt-0.5'>You can no longer update profile</p>
                        </div>
                    </div>
                ) :(
                    <div className="flex justify-end pt-2">
                        <button type='submit' disabled={loading} className='btn-primary flex items-centers gap-2 justify-center w-full sm:w-auto'>
